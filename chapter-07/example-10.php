...
Description: Add "Allow Updates: No" to a plugin's header to stop it from updating.
Version: .1
...
//loop through plugins, check if updates are disallowed and if so remove it from list
function spu_pre_set_site_transient_update_plugins($update_plugins)
{
	...
	if(!empty($update_plugins) && !empty($update_plugins->response))
	{
		 ...
		 foreach($update_plugins->response as $pluginpath => $plugin)
		 {
			 ...
			 $plugin_data = get_plugin_data(ABSPATH . "/wp-content/plugins/" . $pluginpath);
			 if(strtolower($plugin_data['Allow Updates']) == "no" || 
			    strtolower($plugin_data['Allow Updates']) == "false")
			 {
				//change checked version and don't add this plugin to the new response
