...
class SchoolPress_Note_Widget extends WP_Widget 
{
	...
	public function widget( $args, $instance ) 
	{
		...
		if(!empty($_POST['schoolpress_note_text']) && !empty($_POST['class_id']))
		{
			...
			if(groups_is_user_admin($current_user->ID, intval($_POST['class_id'])))
			{
				...
				update_option("schoolpress_note_" . intval($_POST['class_id']), $_POST['schoolpress_note_text']);
			}
		}
		...
		if(!empty($note))
		{
			...
			<div id="schoolpress_note_edit" style="display: none;">
				<form action="" method="post">
					...
					<textarea name="schoolpress_note_text" cols="30" rows="5"><?php echo esc_textarea(get_option("schoolpress_note_" . $class_id));?></textarea>
	...
	?>
	<p>
		<label for="<?php echo $this->get_field_id( 'note' ); ?>"><?php _e( 'Note:' ); ?></label> 
		<textarea id="<?php echo $this->get_field_id( 'note' ); ?>" name="<?php echo $this->get_field_name( 'note' ); ?>"><?php echo esc_textarea($note);?></textarea>
	</p>
	<?php
	}
